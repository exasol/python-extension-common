...
from test.utils.db_utils import (
    create_schema,
    open_schema,
)
...
def container_name() -> str:
    ...
    return "template-Exasol-all-python-3.12-release_x64_11.2.0.tar.gz"


@pytest.fixture(scope="session")
def container_url_formatter(container_name) -> str:
    return (
        "https://github.com/exasol/script-languages-release/releases/" "download/{version}/"
    ) + container_name
...
def main_func(slc_name, slc_url_formatter):
    ...
    slc_parameter_formatters.set_formatter(
        CustomizableParameters.container_url, container_url_formatter
    )
...
def deployer_factory(
    backend_aware_database_params, backend_aware_bucketfs_params, db_schema, language_alias
):
    @contextmanager
    def create_deployer(create_test_schema: bool = False, open_test_schema: bool = True):
        with ExitStack() as stack:
            pyexasol_connection = stack.enter_context(
                pyexasol.connect(**backend_aware_database_params)
            )
...
@pytest.fixture(scope="session")
def onprem_db_params(backend_aware_onprem_database, exasol_config) -> dict[str, Any]:
    return {
...
@pytest.fixture(scope="session")
def onprem_bfs_params(backend_aware_onprem_database, bucketfs_config) -> dict[str, Any]:
...
@pytest.fixture(scope="session")
def saas_params_id(
    saas_host, saas_pat, saas_account_id, backend_aware_saas_database_id
) -> dict[str, Any]:
    return {
...
@pytest.fixture(scope="session")
def saas_params_name(saas_params_id, database_name) -> dict[str, Any]:

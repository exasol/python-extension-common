...
def create_deploy_command(
    ...
    container_url_formatter: str | None = None,
) -> click.Command:
    """
    ...
    opts = select_std_options(
        [StdTags.DB | backend_tag, StdTags.BFS | backend_tag, StdTags.SLC], formatters=formatters
    )
...
def run_deploy_command(
    ...
    with ExitStack() as stack:
        ...
        runner = CliRunner()
        runner.invoke(
            deploy_command, args=arg_string, catch_exceptions=False, standalone_mode=False
        )
...
def test_slc_deployer_cli_onprem_url(
    use_onprem,
    container_version,
    container_name,
    ...
    deploy_command = create_deploy_command(
        StdTags.ONPREM,
...
def test_slc_deployer_cli_onprem_file(
    use_onprem, container_path, language_alias, db_schema, onprem_cli_args, slc_cli_args
):
    if not use_onprem:
        pytest.skip("The test is not configured to use ITDE.")
...
def test_slc_deployer_cli_saas_url(
    use_saas,
    container_version,
    container_name,
    container_url_formatter,
    ...
    deploy_command = create_deploy_command(
        StdTags.SAAS, container_name=container_name, container_url_formatter=container_url_formatter
    )
    extra_cli_args = {StdParams.version.name: container_version}
...
def test_slc_deployer_cli_saas_file(
    use_saas, container_path, language_alias, db_schema, saas_cli_args, slc_cli_args
):
    if not use_saas:
        pytest.skip("The test is not configured to run in SaaS.")
